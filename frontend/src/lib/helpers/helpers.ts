...
export const formatDurationFlexible = (seconds: number): { time: string; isLong: boolean } => { 
  ...
  const s = seconds % 60; 
  if (h > 0) { 
    return { time: `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`, isLong: true }; 
  } else { 
    return { time: `${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}`, isLong: false }; 
  } 
};
...
export const adaptPostToUser = (post: { 
  ...
  id: typeof post.id === 'number' ? post.id : parseInt(post.id || '0', 10), 
  userId: String(post.userId || '0'),
  name: post.userName || '用户', 
  avatar: getAvatarUrl(post.userAvatar || ''), 
  message: (post.content && post.content.trim()) ? post.content : ' ', 
  likes: post.likes || 0, 
  comments: (post.comments || []).map(c => ({ 
    id: typeof c.id === 'number' ? c.id : parseInt(c.id || '0', 10), 
    userId: String(c.userId || '0'),
    userName: c.userName || '用户', 
    userAvatar: getAvatarUrl(c.userAvatar || ''), 
    content: c.content || '', 
    time: formatTimeAgo(c.createdAt || new Date().toISOString()) 
  })), 
  totalDays: 0, 
  completedFlags: 0, 
  totalPoints: 0, 
...
export function calculateTaskCompletionPoints(params: { priority: 1 | 2 | 3 | 4; label: 1 | 2 | 3 | 4 | 5; total: number; }): number { 
...
export function validatePunch(lastPunchTime: number, now: number = Date.now()): { valid: boolean; message?: string; } { 
  if (!lastPunchTime) { 
    return { valid: true }; 
  } 
  ...
  if (hoursSinceLastPunch < PUNCH_LIMITS.MIN_INTERVAL_HOURS) { 
    ...
    return { valid: false, message: `请等待${hoursLeft}小时后再打卡` }; 
  } 
  return { valid: true }; 
}
...
export function getPointsRank(totalPoints: number): { rank: string; minPoints: number; maxPoints: number; } { 
  const ranks = [
...
export async function withErrorHandling<T>(fn: () => Promise<T>, errorMessage = '操作失败'): Promise<T> { 
  try { 
    return await fn(); 
  } catch (error) { 
...
export async function batchOperation<T, R>(items: T[], operation: (item: T) => Promise<R>, onProgress?: (completed: number, total: number) => void): Promise<R[]> { 
  const results: R[] = []; 
