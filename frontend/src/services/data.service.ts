...
export async function getMonthlyStats(): Promise<MonthlyStats> {
  ...
  try {
    const [_dakaTotal, monthDaka, learnTime] = await Promise.all([
      api.get<{ dakaTotal: number }>('/api/getdakatotal'),
      api.get<{ monthDaka: number }>('/api/getmonthdaka'),
      api.get<{ monthLearntime: number }>('/api/getLearnTimemonly')
    ]);
    
    const punchedDays = monthDaka.monthDaka || 0;
    
    return {
      punchedDays,
      missedDays: Math.max(0, new Date().getDate() - punchedDays),
      totalStudyTime: learnTime.monthLearntime || 0, // 后端已修正为秒
    };
  } catch (error) {
...
export async function getFlagLabels(): Promise<Array<{ label: number; count: number; name: string }>> {
  const { api } = await import('./apiClient');
  try {
    const response = await api.get<{ label: { userId: number; life: number; study: number; work: number; likeCount: number; sport: number } }>('/api/getlabel');
    ...
    return [
      ...
      { label: 4, count: labels.likeCount, name: labelNames[3] },
...
export async function getStudyTimeTrend(period: 'week' | 'month' | 'year'): Promise<StudyTimeTrend[]> {
  ...
  try {
    ...
    const result = response.learnTimes.map((item) => {
      // createdAt 格式: "2025-11-01T00:00:00+08:00" 或 "2025-11-01"
      // 直接提取YYYY-MM-DD部分，不经过Date对象
      const dateStr = item.createdAt.split('T')[0]; // 取T之前的部分
      return {
        date: dateStr,
